#!/usr/bin/env node
/**
 * Sync etude/jewelry → public/jewelry (mid home marquee).
...
const outDir = path.join(root, "public/jewelry");
const cacheDir = path.join(root, ".cache/jewelry-processed");
...
function hasCommittedJewelry() {
  try {
    return fs.readdirSync(outDir).some((f) => /^jewel-\d+\.jpg$/i.test(f));
  } catch {
    return false;
  }
}
...
(async () => {
  if (!sourceDirs.length) {
    console.warn("sync-jewelry: no source folder — keeping existing public/jewelry");
    process.exit(0);
  }

  const files = collectFiles();
  if (!files.length) {
    console.warn("sync-jewelry: no images found");
    process.exit(0);
  }

  if (!IS_MAC && process.env.VERCEL && hasCommittedJewelry()) {
    console.log(
      "sync-jewelry: Vercel build — using committed public/jewelry (run sync locally on Mac)"
    );
    process.exit(0);
  }
  ...
  for (const f of fs.readdirSync(outDir)) {
    if (/^jewel-\d+\.jpg$/i.test(f)) fs.unlinkSync(path.join(outDir, f));
  }

  let i = 0;
  for (const abs of files) {
    i++;
    const out = path.join(outDir, `jewel-${String(i).padStart(2, "0")}.jpg`);
  ...
  console.log(
    `sync-jewelry: ${files.length} image(s) · engine: ${IS_MAC ? "macOS Vision (clean)" : "fallback"}`
  );
})().catch((e) => {
  console.error(e);
  process.exit(1);
