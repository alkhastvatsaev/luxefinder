#!/usr/bin/env node
/**
 * Sync pics/ → public/pics (carousel).
 * macOS 14+: native Vision subject lift (same tech as Photos) — no extra post-process.
 * Linux/Vercel: keep committed public/pics, else flood-fill fallback.
 */
...
const outDir = path.join(root, "public/pics");
const cacheDir = path.join(root, ".cache/pics-processed");
...
function hasCommittedPics() {
  try {
    return fs.readdirSync(outDir).some((f) => /^item-\d+\.jpg$/i.test(f));
  } catch {
    return false;
  }
}
...
(async () => {
  if (!sourceDirs.length) {
    console.warn("sync-pics: no source folder — keeping existing public/pics");
    process.exit(0);
  }

  const files = collectFiles();
  if (!files.length) {
    console.warn("sync-pics: no images found");
    process.exit(0);
  }

  if (!IS_MAC && process.env.VERCEL && hasCommittedPics()) {
    console.log("sync-pics: Vercel build — using committed public/pics (run sync locally on Mac)");
    process.exit(0);
  }

  fs.mkdirSync(outDir, { recursive: true });
  for (const f of fs.readdirSync(outDir)) {
    if (/^item-\d+\.jpg$/i.test(f)) fs.unlinkSync(path.join(outDir, f));
  }

  let i = 0;
  for (const abs of files) {
    i++;
    const out = path.join(outDir, `item-${String(i).padStart(2, "0")}.jpg`);
  ...
  console.log(
    `sync-pics: ${files.length} image(s) · engine: ${IS_MAC ? "macOS Vision (clean)" : "fallback"}`
  );
})().catch((e) => {
  console.error(e);
  process.exit(1);
